...
def run_floyd_warshall(
    ...
    steps.append({
        "type": "done",
        "path": path,
        "distance": dist[start_node][end_node]
    })
...
def index():
    ...
    return render_template(
        "index.html",
        nodes=sorted(LOCATION_COORDS.keys()),
        edges=RAW_EDGES
    )
...
def route_api():
    ...
    if (
        start not in LOCATION_COORDS
        or
        end not in LOCATION_COORDS
    ):

        return jsonify({
            "error":
                "출발지 또는 목적지가 올바르지 않습니다."
        }), 400
    ...
    traffic_edge = tuple(
        sorted(
            RAW_EDGES[edge_index]
        )
    )

    algorithm_function = ALGORITHMS.get(
        algorithm
    )
